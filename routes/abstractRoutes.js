...
const multer = require("multer");
const Abstract = require("../models/abstractmodel");

// // Multer memory storage (same as your setup)
// const upload = multer({ storage: multer.memoryStorage() });
const upload = multer({
 ...
 */
router.post("/abstract-submission", upload.single("abstract"), async (req, res) => {
  try {
    const form = req.body;
    const abstractFile = req.file;

    if (!abstractFile || !form.websiteDomain) {
      return res.status(400).json({ message: "PDF file & websiteDomain are required" });
    }
    ...
    res.status(500).json({ message: "Failed to submit abstract" });
  }
});

/**
 * GET: All Abstracts (Admin Dashboard)
 */
...
/**
 * GET: Abstracts by Website Domain
 */
router.get("/abstracts/domain/:domain", async (req, res) => {
  try {
 ...
 * GET: Download PDF
 */
router.get("/abstracts/file/:id", async (req, res) => {
  try {
    const record = await Abstract.findById(req.params.id);

    if (!record || !record.abstractFile?.data) {
      return res.status(404).json({ message: "File not found" });
    }

    res.set("Content-Type", record.abstractFile.contentType);
    res.set(
      "Content-Disposition",
      `attachment; filename="${record.abstractFile.filename}"`
    );

    res.send(record.abstractFile.data);
  } catch (error) {
