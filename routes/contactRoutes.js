...
router.post("/contact", upload.none(), async (req, res) => {
  try {
    const { firstName, lastName, email, phone, company, message, websiteDomain } = req.body;

    // Basic validation
    if (!firstName || !lastName || !email || !message || !websiteDomain) {
    ...
    res.status(200).json({ message: "Form submitted successfully!",newContact });
  } catch (error) {
...
router.get("/contacts", async (req, res) => {
  try {
    const contacts = await ContactModel.find().sort({ createdAt: -1 }); // latest first
...
// --- Optional GET: Filter by websiteDomain ---
router.get("/contacts/:domain", async (req, res) => {
  try {
    const { domain } = req.params;
    const contacts = await ContactModel.find({ websiteDomain: domain }).sort({ createdAt: -1 });
    res.status(200).json(contacts);
  } catch (error) {
    console.error("Error fetching contacts by domain:", error);
    res.status(500).json({ message: "Failed to fetch contacts for this domain." });
  }
});
